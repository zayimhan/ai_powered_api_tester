...
const { scenarioTable, scenarioStepTable } = require('../models/scenario.model');
...
try { db.exec('ALTER TABLE collections ADD COLUMN user_id INTEGER REFERENCES users(id)'); } catch (e) { }
try { db.exec('ALTER TABLE saved_requests ADD COLUMN user_id INTEGER REFERENCES users(id)'); } catch (e) { }
try { db.exec('ALTER TABLE execution_history ADD COLUMN user_id INTEGER REFERENCES users(id)'); } catch (e) { }

// Migration: Add request details to execution_history for history re-run
try { db.exec('ALTER TABLE execution_history ADD COLUMN method TEXT'); } catch (e) { }
try { db.exec('ALTER TABLE execution_history ADD COLUMN url TEXT'); } catch (e) { }
try { db.exec('ALTER TABLE execution_history ADD COLUMN request_headers TEXT'); } catch (e) { }
try { db.exec('ALTER TABLE execution_history ADD COLUMN request_body TEXT'); } catch (e) { }
